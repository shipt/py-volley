...
#     )
#     # confluent producer needs consumer's on_success and on_fail, these are set by volley
#     def dummy_callback(context: Any) -> None:
#         pass
#     producer.set_on_fail(dummy_callback)
#     producer.set_on_success(dummy_callback)
...
def test_confluent_async_consume(
    ...
    producer1.init_callbacks(on_fail=consumer1.on_fail, on_success=consumer1.on_success, thread=False)
    time.sleep(1)
    producer1.produce(
        queue_name=environment.test_topic, message="message".encode("utf-8"), message_context=message_0.message_context
    )
    ...
    assert consumer2.last_offset == {}
    # init the callbacks
    producer2.init_callbacks(on_fail=consumer2.on_fail, on_success=consumer2.on_success)
